...
def get_snapshot(task):
    timestamp, camera_code, camera_name = task
    ...
    try:
        ...
        print(f"[{multiprocessing.current_process().name}] {file_name}")
        rtsp_url = retrieve_rtsp(camera_code, timestamp_ns)
        capture_screenshot(rtsp_url, folder_name, file_name)

    except Exception as e:
        error_msg = f"[ERROR] Failed snapshot for {camera_name} @ {timestamp}: {e}"
        print(error_msg)
        log_error_to_file(error_msg)
...
if __name__ == "__main__":
    try:
        ...
        start_timestamp = 1741363200
        end_timestamp = 1741449600
        sample = 4  # sample between one hour
        gap = int(3600 / sample)
        timestamps = list(range(start_timestamp, end_timestamp, gap))

        # Flatten the task list
        tasks = [
            (ts, row['cameraIndexCode'], row['name'])
            for ts in timestamps
            for _, row in df.iterrows()
        ]

        with multiprocessing.Pool(processes=80) as pool:
            pool.map(get_snapshot, tasks)
